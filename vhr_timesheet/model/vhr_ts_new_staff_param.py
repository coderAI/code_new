...
class vhr_ts_new_staff_param(osv.osv):
    _name = 'vhr.ts.new.staff.param'
    _description = 'VHR TS New Staff Parameter'
    ...
    def _validation(self, from_date, to_date, coef):
        if from_date and from_date not in xrange(1, 32) \
            ...
            raise osv.except_osv('Validation Error !',
                                 '(C) From Date and To Date must in range of [1-31]!')
        ...
        if coef:
            if coef < 0 or coef > 1:
                raise osv.except_osv('Validation Error !',
                                     '(C) Coef must be in range of [0, 1]!')

    def create(self, cr, uid, vals, context=None):
        self._validation(vals.get('from_date'), vals.get('to_date'), vals.get('coef'))

        return super(vhr_ts_new_staff_param, self).create(cr, uid, vals, context=context)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            if vals.get('from_date') and vals['from_date'] > record.to_date \
                ...
                raise osv.except_osv('Validation Error !',
                                     '(C) From Date must be less than To Date!')
        return super(vhr_ts_new_staff_param, self).write(cr, uid, ids, vals, context=context)


vhr_ts_new_staff_param()