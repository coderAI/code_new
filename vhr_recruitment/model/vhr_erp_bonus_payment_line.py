...
class vhr_erp_bonus_payment_line(osv.osv):
    _name = 'vhr.erp.bonus.payment.line'
    _description = 'VHR ERP Bonus Payment Line'
...
#    def _month_of_paid(self, cr, uid, ids, fields, args, context=None):
#        result = {}
#        for data in self.browse(cr, uid, ids, context=context):
#            result[data.id]={
#             'month_of_paid':False,
...
#            payment_date = data.payment_date
#            date1 = date_now.strftime('%Y-%m-25'),
#            date2 = date_now + relativedelta.relativedelta(months=-1, day=26, days=-1),
#            payment_date_temp =  datetime.strptime(payment_date,'%Y-%m-%d').strftime('%Y-%m-%d')
#            if payment_date_temp >= date2 and payment_date_temp <= date1:
#                temp = datetime.strptime(payment_date,'%Y-%m-%d').strftime('%Y-%m')
#            elif payment_date_temp >= date2 and payment_date_temp >= date1:
#                temp = payment_date + relativedelta.relativedelta(months=+1),
#            elif payment_date_temp <= date2 and payment_date_temp <= date1:
#                temp = payment_date + relativedelta.relativedelta(months=-1),
#            result[data.id]['month_of_paid'] =  temp
#        return result 
    ...
    _columns = {
        'bonus_payment_id': fields.many2one('vhr.erp.bonus.payment', 'Bonus Payment', ondelete='cascade'),
...
#                    multi="month_of_paid"),
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        res = False
        try:
            res = super(vhr_erp_bonus_payment_line, self).unlink(cr, uid, ids, context)
        except Exception as e:
            log.exception(e)
            raise osv.except_osv('Validation Error !', 'You cannot delete the record(s) which reference to others !')
        return res

vhr_erp_bonus_payment_line()
